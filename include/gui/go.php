...
if(!isset($_GET['ven'])) {die('Something goes really wrong.'); }  
if(!isset($_GET['model'])) {die('Something goes really wrong.'); }
if ($_GET['ven']=="#") {die('Something goes really wrong.'); }
if ($_GET['model']=="#") {die('Something goes really wrong.'); }
...
db::connect(); // connect do mysql database
	...
	$result = db::query("SELECT * FROM `{prefix}main` WHERE (`Main.Brand` = '".$_GET['ven']."' AND `Main.Model` = '".$_GET['model']."')");
	while ($inf = db::fetch($result,'BOTH')) { 
...
<?php
			...
			printf('<h3>%s</h3><div><table style="position:absolute;left:27px;top:17px;width:701px;z-index:0;">',$spec_groups[0]); 
				...
				while ($spec_name = db::fetch($result1,'ROW')) { 
						...
						$result2 = db::query("SELECT `".$spec_groups[0].".".$spec_name[0]."` FROM `{prefix}main` WHERE (`Main.Brand` = '".$_GET['ven']."' AND `Main.Model` = '".$_GET['model']."')");
